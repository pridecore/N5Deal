...
export default async function MarketplacePage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <div className="noise min-h-screen"><div className="fine-grid absolute inset-0 opacity-50" aria-hidden="true" /><div className="market-shell">
    <div className="border-b border-[#d9d4c9] pb-6">
      <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="eyebrow rise">Marketplace / active inventory</p><h1 className="display compact-heading mt-3 text-[#172532] rise rise-delay">Find the right regulated deal flow.</h1></div><div className="max-w-[360px] text-sm leading-6 text-[#50606a] rise rise-delay-2"><p>Banking, payments, fintech, EMI and crypto opportunities presented as structured acquisition briefs.</p><p className="mt-3 stat-label">{pagination.total} active {pagination.total === 1 ? "opportunity" : "opportunities"}</p></div></div>
      <nav aria-label="Marketplace categories" className="mt-6 flex gap-2 overflow-x-auto pb-1">
        <Link href={categoryHref(filters)} aria-current={!filters.category ? "page" : undefined} className={`focus-ring deal-badge shrink-0 ${!filters.category ? "status-live" : ""}`}>All</Link>
    ...
    <div className="mt-7 grid gap-7 lg:grid-cols-[280px_1fr] lg:items-start">
      ...
      {result.items.length === 0 ? <section className="border-b border-[#d9d4c9] py-20 text-center"><p className="eyebrow">No matches</p><h2 className="display mt-4 text-4xl tracking-[-.04em]">The room is quiet.</h2><p className="mx-auto mt-4 max-w-[380px] text-sm leading-6 text-[#50606a]">Try widening your search or resetting the filters to see every published opportunity.</p><Link href="/marketplace" className="focus-ring mt-7 inline-block border-b border-[#b7653b] pb-2 text-[10px] font-bold uppercase tracking-[.14em] text-[#b7653b]">Reset filters ↗</Link></section> : <section aria-label="Marketplace listings">
        <div className="mb-4 flex items-center justify-between border-b border-[#d9d4c9] pb-3"><p className="stat-label">Listings</p><p className="text-[11px] text-[#7a817f]">Page {pagination.page} of {pagination.pageCount || 1}</p></div>
        <div className="grid gap-4 xl:grid-cols-2">{result.items.map((asset) => <AssetCard key={asset.id} asset={asset as unknown as AssetCardData} />)}</div>
