...
export function AssetForm({ mode, asset }: AssetFormProps) {
  ...
  return <form onSubmit={handleSubmit(onSubmit)} className="space-y-8" noValidate>
    <div className="grid gap-6 sm:grid-cols-2"><Field label="Asset title" error={errors.title?.message} className="sm:col-span-2"><input {...register("title")} placeholder="e.g. Orbit Payments Ltd" className={inputClass} /></Field><Field label="Category" error={errors.category?.message}><select {...register("category")} className={inputClass}>{assetCategories.map((value) => <option key={value} value={value}>{labelize(value)}</option>)}</select></Field><Field label="Country" error={errors.country?.message}><input {...register("country")} placeholder="Germany" className={inputClass} /></Field><Field label="Asking price" error={errors.askingPrice?.message}><input {...register("askingPrice")} inputMode="decimal" placeholder="12500000.00" className={inputClass} /></Field><Field label="Currency" error={errors.currency?.message}><input {...register("currency")} maxLength={3} placeholder="EUR" className={`${inputClass} uppercase`} /></Field><Field label="Annual revenue" hint="Optional" error={errors.revenue?.message}><input {...register("revenue")} inputMode="decimal" placeholder="5000000.00" className={inputClass} /></Field><Field label="EBITDA" hint="Optional" error={errors.ebitda?.message}><input {...register("ebitda")} inputMode="decimal" placeholder="1200000.00" className={inputClass} /></Field><Field label="Deal type" error={errors.dealType?.message}><select {...register("dealType")} className={inputClass}>{dealTypes.map((value) => <option key={value} value={value}>{labelize(value)}</option>)}</select></Field><Field label="Business status" error={errors.businessStatus?.message}><input {...register("businessStatus")} placeholder="Trading" className={inputClass} /></Field></div>
    <Field label="Business overview" hint="20–5,000 characters" error={errors.description?.message}><textarea {...register("description")} rows={7} placeholder="Describe the business, its position in the market, and the opportunity for a new owner…" className={`${inputClass} resize-y py-3 leading-6`} /></Field>
